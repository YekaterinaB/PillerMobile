...
class MainActivity : ActivityWithUserObject() {
    ...
    fun setCurrentProfileTvVisibility(isVisible:Boolean){
        // current profule is not in play in settings & supervisors
        if(isVisible){
            _currentProfileTV.visibility= View.VISIBLE

        }else{
            _currentProfileTV.visibility= View.GONE
